...
export default function WriteIt({ recordAnswer, onAchievement, onStreak }) {
  ...
  const check = () => {
    ...
    setFeedback({ correct, answer: item.term });
  };
    ...
    <div>
      ...
      <div className="input-mode">
        ...
        <div className="prompt-card">
          ...
          {item.example && <div className="example-hint">💡 {item.example}</div>}
        </div>
        ...
        {feedback && (
          <>
            <div className={`feedback ${feedback.correct ? 'correct' : 'wrong'}`}>
              {feedback.correct ? '✅ Correct!' : <>✗ Not quite<div className="correct-answer">Correct: <strong>{feedback.answer}</strong></div></>}
            </div>
